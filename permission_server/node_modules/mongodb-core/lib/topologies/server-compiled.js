 ...
 * @ignore
 */
var bindToCurrentDomain = function bindToCurrentDomain(callback) {
...
// Supports server
var supportsServer = function supportsServer(_s) {
  return _s.ismaster && typeof _s.ismaster.minWireVersion == 'number';
};

//
// createWireProtocolHandler
var createWireProtocolHandler = function createWireProtocolHandler(result) {
...
//
// Reconnect server
var reconnectServer = function reconnectServer(self, state) {
  ...
  // error handler
  var reconnectErrorHandler = function reconnectErrorHandler(err) {
...
//
// Handlers
var messageHandler = function messageHandler(self, state) {
...
var fatalErrorHandler = function fatalErrorHandler(self, state) {
  return function (err, connection) {
    if (state.state == DISCONNECTED || state.state == DESTROYED) return;
    // Set disconnected state
    state.state = DISCONNECTED;
...
var connectHandler = function connectHandler(self, state) {
  ...
  var applyAuthentications = function applyAuthentications(callback) {
    // We need to ensure we have re-authenticated
    var keys = Object.keys(state.authProviders);
...
var slaveOk = function slaveOk(r) {
...
var notifyStrategies = function notifyStrategies(self, state, op, params, callback) {
...
var debugFields = ['reconnect', 'reconnectTries', 'reconnectInterval', 'emitError', 'cursorFactory', 'host', 'port', 'size', 'keepAlive', 'keepAliveInitialDelay', 'noDelay', 'connectionTimeout', 'socketTimeout', 'singleBufferSerializtion', 'ssl', 'ca', 'cert', 'key', 'rejectUnauthorized', 'promoteLongs'];
...
var executeSingleOperation = function executeSingleOperation(self, ns, cmd, queryOptions, options, onAll, callback) {
  ...
  // Notify query start to any read Preference strategies
  if (self.s.readPreferenceStrategies != null) notifyStrategies(self, self.s, 'startOperation', [self, query, new Date()]);
  ...
  self.s.callbacks.register(query.requestId, function (err, result) {
    ...
    if (result.documents[0]['$err'] || result.documents[0]['errmsg'] || result.documents[0]['err'] || result.documents[0]['code']) return callback(MongoError.create(result.documents[0]));
    // Execute callback, catch and rethrow if needed
    try {
...
Server.prototype.command = function (ns, cmd, options, callback) {
  ...
  // Notify query start to any read Preference strategies
  if (self.s.readPreferenceStrategies != null) notifyStrategies(self, self.s, 'startOperation', [self, queries, new Date()]);
  ...
  for (var i = 0; i < queries.length; i++) {
    ...
    self.s.callbacks.register(query.requestId, function (err, result) {
      ...
      // Return the result
      if (result.documents[0]['$err'] || result.documents[0]['errmsg'] || result.documents[0]['err'] || result.documents[0]['code']) {
...
Server.prototype.auth = function (mechanism, db) {
  ...
  if (self.s.authProviders[mechanism] == null && mechanism != 'default') throw new MongoError(f("auth provider %s does not exist", mechanism));
