...
//
// Initialize step
var GSSAPIInitialize = function GSSAPIInitialize(db, username, password, authdb, gssapiServiceName, server, connection, callback) {
...
var MongoDBGSSAPIFirstStep = function MongoDBGSSAPIFirstStep(mongo_auth_process, payload, db, username, password, authdb, server, connection, callback) {
  ...
  server.command("$external.$cmd", command, { connection: connection }, function (err, r) {
    if (err) return callback(err, false);
    var doc = r.result;
...
//
// Perform first step against mongodb
var MongoDBGSSAPISecondStep = function MongoDBGSSAPISecondStep(mongo_auth_process, payload, doc, db, username, password, authdb, server, connection, callback) {
...
var MongoDBGSSAPIThirdStep = function MongoDBGSSAPIThirdStep(mongo_auth_process, payload, doc, db, username, password, authdb, server, connection, callback) {
  // Build final command
  var command = {
    saslContinue: 1,
    conversationId: doc.conversationId,
...
//# sourceMappingURL=gssapi-compiled.js.map