...
var LegacySupport = function LegacySupport() {};
...
//
var executeOrdered = function executeOrdered(opType, command, ismaster, ns, bson, pool, callbacks, ops, options, callback) {
  ...
  var executeOp = function executeOp(list, _callback) {
    ...
    if (connection == null) return _callback(new MongoError("no connection available"));
    ...
    try {
      ...
      if (hasWriteConcern(writeConcern)) {
        ...
        for (var i = 0; i < writeConcernFields.length; i++) {
          if (writeConcern[writeConcernFields[i]] != null) getLastErrorCmd[writeConcernFields[i]] = writeConcern[writeConcernFields[i]];
        }
...
var executeUnordered = function executeUnordered(opType, command, ismaster, ns, bson, pool, callbacks, ops, options, callback) {
  // Bind to current domain
  callback = bindToCurrentDomain(callback);
...
//# sourceMappingURL=2_4_support-compiled.js.map