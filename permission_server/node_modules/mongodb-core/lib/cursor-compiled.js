...
var execGetMore = function execGetMore(self, callback) {
...
var execInitialQuery = function execInitialQuery(self, query, cmd, options, cursorState, connection, logger, callbacks, callback) {
  if (logger.isDebug()) {
    logger.debug(f("issue initial query [%s] with flags [%s]", JSON.stringify(cmd), JSON.stringify(query)));
  }
...
var nextFunction = function nextFunction(self, callback) {
  ...
  var processExhaustMessages = function processExhaustMessages(err, result) {
    if (err) {
      self.cursorState.dead = true;
  ...
  } else if (self.cursorState.documents.length == self.cursorState.cursorIndex && self.cmd.tailable) {
    return handleCallback(callback, MongoError.create({
      message: "No more documents in tailed cursor",
      tailable: self.cmd.tailable,
      awaitData: self.cmd.awaitData
