...
Model.prototype.save = function (options, fn) {
  ...
  p1.then(this.$__handleSave.bind(this, options)).then(function (result) {
    self.$__reset();
    self.$__storeShard();
...
function checkDivergentArray(doc, path, array) {
  ...
  if (!pop && doc.$__.selected) {
    ...
    if (doc.$__.selected[top] && doc.$__.selected[top].$elemMatch || doc.$__.selected[top + '.$']) {
      return top;
    }
  }
  ...
  var check = pop.options.match || pop.options.options && hasOwnProperty(pop.options.options, 'limit') || // 0 is not permitted
  pop.options.options && pop.options.options.skip || // 0 is permitted
  pop.options.select && ( // deselected _id?
  0 === pop.options.select._id || /\s?-_id\s?/.test(pop.options.select));
...
for (var i in EventEmitter.prototype) Model[i] = EventEmitter.prototype[i];
...
function isDoc(doc) {
  ...
  if (Buffer.isBuffer(doc)) return false;

  if ('ObjectID' == doc.constructor.name) return false;
...
Model._getSchema = function _getSchema(path) {
  ...
  return (function search(_x, _x2) {
    ...
    _function: while (_again) {
      var parts = _x,
          schema = _x2;
      p = foundschema = trypath = undefined;
      _again = false;
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  // generate new class
  function model(doc, fields, skipId) {
    if (!(this instanceof model)) return new model(doc, fields, skipId);
 ...
 * @param {Schema} schema
 */
var applyStatics = function applyStatics(model, schema) {
...
Model.__subclass = function subclass(conn, schema, collection) {
  ...
  var s = schema && 'string' != typeof schema ? schema : model.prototype.schema;

  var options = s.options || {};

  if (!collection) {
    collection = model.prototype.schema.get('collection') || utils.toCollectionName(model.modelName, options);
  }
