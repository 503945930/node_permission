...
reserved.emit = reserved.on = reserved.once = // EventEmitter
reserved.db = reserved.set = reserved.get = reserved.init = reserved.isNew = reserved.errors = reserved.schema = reserved.modelName = reserved.collection = reserved.toObject = reserved.save = reserved.validate = reserved.increment = // document properties and functions
...
Schema.interpretAsType = function (path, obj) {
  ...
  if (Array.isArray(type) || Array == type || 'array' == type) {
    ...
    } else if (cast && (!cast.type || cast.type.type) && 'Object' == utils.getFunctionName(cast.constructor) && Object.keys(cast).length) {
...
Schema.prototype.method = function (name, fn) {
  if ('string' != typeof name) for (var i in name) this.methods[i] = name[i];else this.methods[name] = fn;
  return this;
};
...
Schema.prototype['static'] = function (name, fn) {
  if ('string' != typeof name) for (var i in name) this.statics[i] = name[i];else this.statics[name] = fn;
  return this;
};
...
Schema.prototype.set = function (key, value, _tags) {
  ...
  switch (key) {
    case 'read':
      ...
      break;
    case 'safe':
      this.options[key] = false === value ? { w: 0 } : value;
      break;
    default:
    ...
    for (var i = 0; i < keys.length; ++i) {
      ...
      if (path instanceof MongooseTypes.DocumentArray) {
        ...
        if (false !== index && null != index) {
          ...
          type = 'string' == typeof index ? index : isObject ? index.type : false;
