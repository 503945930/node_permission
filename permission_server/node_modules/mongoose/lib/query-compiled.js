...
Query.prototype.find = function (conditions, callback) {
  ...
  var cb = function cb(err, docs) {
    ...
    if (!options.populate) {
      return true === options.lean ? promise.complete(docs) : completeMany(self.model, docs, fields, self, null, promise);
    }

    var pop = helpers.preparePopulationOptionsMQ(self, options);
    self.model.populate(docs, pop, function (err, docs) {
      if (err) return promise.error(err);
      return true === options.lean ? promise.complete(docs) : completeMany(self.model, docs, fields, self, pop, promise);
    });
  };
...
Query.prototype.findOne = function (conditions, projection, options, callback) {
  ...
  this.model.hooks.execPre('findOne', this, function (error) {
    ...
    Query.base.findOne.call(self, {}, function (err, doc) {
      ...
      if (!options.populate) {
        return true === options.lean ? promise.complete(doc) : completeOne(self.model, doc, projection, self, null, promise);
      }
      ...
      self.model.populate(doc, pop, function (err, doc) {
        ...
        return true === options.lean ? promise.complete(doc) : completeOne(self.model, doc, projection, self, pop, promise);
      });
    });
  });
...
Query.prototype.findOneAndUpdate = function () {
  var hasCallback = arguments.length && typeof arguments[arguments.length - 1] === 'function';

  if (hasCallback && this.schema) {
    this.model.hooks.wrap('findOneAndUpdate', Query.base.findOneAndUpdate, this, Array.prototype.slice.call(arguments));
  } else {
...
Query.prototype.findOneAndRemove = function () {
  ...
  if (hasCallback && this.schema) {
    this.model.hooks.wrap('findOneAndRemove', Query.base.findOneAndRemove, this, Array.prototype.slice.call(arguments));
  } else {
...
Query.prototype._findAndModify = function (type, callback) {
  ...
  var cb = function cb(err, doc) {
    ...
    if (!options.populate) {
      return true === options.lean ? promise.complete(doc) : completeOne(self.model, doc, fields, self, null, promise);
    }
    ...
    self.model.populate(doc, pop, function (err, doc) {
      ...
      return true === options.lean ? promise.complete(doc) : completeOne(self.model, doc, fields, self, pop, promise);
    });
  };
...
Query.prototype._castUpdate = function _castUpdate(obj, overwrite) {
  ...
  while (i--) {
    ...
    if (val && 'Object' === val.constructor.name && !overwrite) {
      ...
      var msg = 'Invalid atomic update value for ' + op + '. ' + 'Expected an object, received ' + typeof val;
      throw new Error(msg);
    }
  }
...
Query.prototype._walkUpdatePath = function _walkUpdatePath(obj, op, pref) {
  ...
  var strict = 'strict' in this._mongooseOptions ? this._mongooseOptions.strict : this.model.schema.options.strict;
  ...
  while (i--) {
    ...
    if (val && 'Object' === val.constructor.name) {
      ...
      if (schema && schema.caster && op in castOps) {
        ...
        hasKeys |= this._walkUpdatePath(val, op, prefix + key) || utils.isObject(val) && Object.keys(val).length === 0;
      }
    } else {
      schema = '$each' === key || '$or' === key || '$and' === key ? this._getSchema(pref) : this._getSchema(prefix + key);
...
Query.prototype.near = function () {
  ...
  if (arguments.length === 1) {
    ...
    if ('string' == typeof arguments[0] && 'number' == typeof arguments[1] && 'number' == typeof arguments[2]) {
      params.push(arguments[0]);
