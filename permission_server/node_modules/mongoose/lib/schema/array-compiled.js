...
function SchemaArray(key, cast, options) {
  if (cast) {
    ...
    // support { type: 'String' }
    var name = 'string' == typeof cast ? cast : utils.getFunctionName(cast);
...
SchemaArray.prototype.castForQuery = function ($conditional, value) {
  ...
  if (arguments.length === 2) {
    ...
    if (Array.isArray(val)) {
      val = val.map(function (v) {
        ...
        return isMongooseObject(v) ? v.toObject({ virtuals: false }) : v;
      });
    } else if (method) {
  ...
  return val && isMongooseObject(val) ? val.toObject({ virtuals: false }) : val;
};
...
handle.$near = handle.$nearSphere = cast$near;

handle.$within = handle.$geoWithin = cast$within;

handle.$size = handle.$maxDistance = castToNumber;
