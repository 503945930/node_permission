...
function QueryStream(query, options) {
  ...
  this._inline = T_INIT;
  this._running = false;
  this._transform = options && 'function' == typeof options.transform ? options.transform : K;
...
QueryStream.prototype.__next = function () {
  if (this.paused || this._destroyed) return this._running = false;
...
QueryStream.prototype._onNextObject = function _onNextObject(err, doc) {
  ...
  if (!opts.populate) {
    return true === opts.lean ? emit(this, doc) : createAndEmit(this, null, doc);
  }
  ...
  self.query.model.populate(doc, pop, function (err, doc) {
    if (err) return self.destroy(err);
    return true === opts.lean ? emit(self, doc) : createAndEmit(self, pop, doc);
  });
};
...
//# sourceMappingURL=querystream-compiled.js.map