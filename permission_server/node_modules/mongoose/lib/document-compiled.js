...
function init(self, obj, doc, prefix) {
  ...
  while (len--) {
    ...
    if (!schema && utils.isObject(obj[i]) && (!obj[i].constructor || 'Object' == utils.getFunctionName(obj[i].constructor))) {
...
Document.prototype.set = function (path, val, type, options) {
    ...
    if (null === path || undefined === path) {
      ...
      while (i--) {
        ...
        // need to know if plain object - no Buffer, ObjectId, ref, etc
         && utils.isObject(path[key]) && (!path[key].constructor || 'Object' == utils.getFunctionName(path[key].constructor)) && 'virtual' != pathtype && !(this.$__path(pathName) instanceof MixedSchema) && !(this.schema.paths[pathName] && this.schema.paths[pathName].options.ref)) {
  ...
  if ('nested' == pathType && val) {
    if (utils.isObject(val) && (!val.constructor || 'Object' == utils.getFunctionName(val.constructor))) {
  ...
  try {
    ...
    this.invalidate(path, new MongooseError.CastError(schema.instance, val, path));
    shouldSet = false;
  }
...
Document.prototype.validate = function (cb) {
  ...
  for (var i = 0; i < paths.length; ++i) {
    ...
    if (val instanceof Array && !Buffer.isBuffer(val) && !val.isMongooseDocumentArray) {
...
function compile(tree, proto, prefix) {
  ...
  while (i--) {
    ...
    defineKey(key, 'Object' === utils.getFunctionName(limb.constructor) && Object.keys(limb).length && (!limb.type || limb.type.type) ? limb : null, proto, prefix, keys);
  }
};
...
Document.prototype.$toObject = function (options, json) {
  ...
  if (!(options && 'Object' == utils.getFunctionName(options.constructor)) || options && options._useSchemaOptions) {
    if (json) {
